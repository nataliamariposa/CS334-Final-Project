...
/// Should correctly handle multibyte character encodings.
/// </summary>
/// <param name="c">A <c>char</c>.</param>
/// <returns>A <c>string</c> representing a char code, in hex.</returns>
let cToHex (c: char) =
    if int c <= 0x7F then
        // ASCII: Use a simple 2-digit hex format
        sprintf "0x%02x" (int c)
    else
        // Non-ASCII: Use UTF-32 encoding
        let bytes = System.Text.Encoding.UTF32.GetBytes([|c|])
        "0x" + System.BitConverter.ToString(bytes).Replace("-", "").ToLower().TrimStart('0')
...
let (<!>)(p: Parser<'a>)(label: string)(i: Input) : Outcome<'a> =
    ...
    if (isDebug i) then
        ...
        if (input i).Length - (position i) > 0 then
            eprintfn "[attempting: %s on \"%s\", pos: %d, next char: %s]" label nextText (position i) (cToHex (input i).[0])
        else
            eprintfn "[attempting: %s on \"%s\", pos: %d, next char: EOF]" label nextText (position i)
        let o = p i
        match o with
        | Success(a, i') ->
...
let cause(p: Parser<'a>)(rule: String)(i: Input) : Outcome<'a> =
    ...
    match o with
    | Success _ -> o
    // note: we need to return the original position where
    // p started, since p may have, along the way, consumed more input
    | Failure(_,_) -> Failure(position i, rule)
...
// pads s to length w on the left
let padleft (s: string)(w: int) : string =
    assert (String.length s <= w)
    let rem = w - String.length s
    s + String.replicate rem " "
...
let diagnosticMessage(window_sz: int)(failure_pos: int)(buffer: string)(err: string) : string =
    ...
    let caret_pos =
        if last_nl_left = 0 then
            failure_pos
        else
            failure_pos - last_nl_left + 1
