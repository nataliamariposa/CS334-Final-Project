...
let pwhile =
    pright (pad (pstr "while")) (
        pseq pexpr pparens (fun (cond, body) -> While (cond, body))
    ) <!> "while"
...
let acceptablecomparisons = psum <|> num <|> pvar <|> strng
...
pexprImpl := pplayerExpr <|> pdeckExpr <|> pwinner <|> pif <|> pprompt <|> phascards <|> pwhile <|> ptakecard <|> pshuffle <|> passign <|> pequal <|> pgreater <|> plesser <|> psum <|>  pfirstcard <|> pshowcard <|> pbooliscool <|> pvar <|> num <|> strng
